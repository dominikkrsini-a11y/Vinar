...
The startup update script already runs `npm install` in both the repo root and `server/` (each has its own lockfile). You should not need to reinstall.
...
- `.env` files are git-ignored. Templates: `.env.example` (root, `EXPO_PUBLIC_*`) and `server/.env.example` (server). Copy and fill them.
